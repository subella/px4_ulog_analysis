...
log_dir = pathlib.Path("../11_6_gripper_logs").resolve()
...
def plot_sp(ax, actual_df, desired_df, actual_labels, desired_labels, start=0.0, end=1000.0):
    ...
    for i, l in enumerate(desired_labels):
        ax.plot(desired_to_plot["times"], desired_to_plot[l], label="{} (setpoint)".format(l), c=colors[i], ls="--")
...
for i, time in enumerate(input_df["times"]):
    if input_df.iloc[i]["current.type"] == 0 and input_df.iloc[i]["current.velocity_valid"]:
...
if len(trajectory_times) > 0 and not show_all:
    ...
    start_time = get_df(data_frames, "vehicle_attitude_0", first_timestamp).iloc[0]["times"]
    end_time = get_df(data_frames, "vehicle_attitude_0", first_timestamp).iloc[-1]["times"]
...
local_pose_sp_df = get_df(data_frames, "vehicle_local_position_setpoint_0", first_timestamp)
...
plot_sp(ax[0][0], local_pose_df, local_pose_sp_df, ["x", "y", "z"], ["x", "y", "z"], start=start_time, end=end_time)    
ax[0][0].set_title("Position Control")
# ax[0][0].legend()
ax[0][0].set_xlabel("Time (seconds)")
ax[0][0].set_ylabel("Position (meters)")

plot_sp(ax[0][1], local_pose_df, local_pose_sp_df, ["vx", "vy", "vz"], ["vx", "vy", "vz"], start=start_time, end=end_time)    
ax[0][1].set_title("Velocity Control")
# ax[0][1].legend()
ax[0][1].set_xlabel("Time (seconds)")
ax[0][1].set_ylabel("Velocity (meters/second)")

plot_sp(ax[1][0], attitude_df, attitude_sp_df, ["q[0]", "q[1]", "q[2]", "q[3]"], ["q_d[0]", "q_d[1]", "q_d[2]", "q_d[3]"], start=start_time, end=end_time)    
ax[1][0].set_title("Attitude Control")
ax[1][0].legend()
ax[1][0].set_xlabel("Time (seconds)")
ax[1][0].set_ylabel("Attitude (radians)")

plot_sp(ax[1][1], attitude_df, attitude_sp_df, ["rollspeed", "pitchspeed", "yawspeed"], ["angle_vel_x", "angle_vel_y", "angle_vel_z"], start=start_time, end=end_time)    
ax[1][1].set_title("Angular Velocity Control")
ax[1][1].legend()
ax[1][1].set_xlabel("Time (seconds)")
ax[1][1].set_ylabel("Angular Velocity (radians/second)")

plot_adaptive(ax[2][0], attitude_sp_df, ["adap_x", "adap_y", "adap_z"], start=start_time, end=end_time)
...
plot_adaptive(ax[2][1], attitude_output_df, ["adap_x", "adap_y", "adap_z"], start=start_time, end=end_time)
...
fig, ax = plt.subplots()
plot_adaptive(ax, attitude_sp_df, ["adap_vx", "adap_vy", "adap_vz"], start=start_time, end=end_time, vel=True)
fig.set_size_inches([12, 6])
plt.show()

attitude_sp_df.head()

attitude_output_df.head()

plt.plot(local_pose_df["times"])
plt.show()

attitude_sp_df.head()

# +
#sns.displot(attitude_output_df, x="dt")
#plt.show()

# +
#sns.displot(attitude_sp_df, x="dt")
#plt.show()
# -

print(attitude_sp_df.columns)

# +
import numpy as np
import functools

def get_matrices(df, name, shape):
...
q_d = get_matrices(attitude_sp_df, "q_d", (4, 1))

print(attitude_sp_df.iloc[100]["q_d[3]"])
# -
