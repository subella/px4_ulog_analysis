...
def plot_sp(ax, actual_df, desired_df, actual_labels, desired_labels, start=0.0, end=1000.0):
    ...
    for i, l in enumerate(desired_labels):
        ax.plot(desired_df["times"], desired_df[l], label="{} (setpoint)".format(l), c=colors[i], ls="--")
...
plot_sp(ax[0], local_pose_df, local_pose_sp_df, ["x", "y", "z"], ["x", "y", "z"], start=start_time, end=end_time)    
ax[0].set_title("Position Control")
ax[0].legend()
ax[0].set_xlabel("Time (seconds)")
ax[0].set_ylabel("Position (meters)")

plot_sp(ax[1], local_pose_df, local_pose_sp_df, ["vx", "vy", "vz"], ["vx", "vy", "vz"], start=start_time, end=end_time)    
...
plot_sp(ax[0], attitude_df, attitude_sp_df, ["q[0]", "q[1]", "q[2]", "q[3]"], ["q_d[0]", "q_d[1]", "q_d[2]", "q_d[3]"], start=start_time, end=end_time)    
ax[0].set_title("Attitude Control")
ax[0].legend()
ax[0].set_xlabel("Time (seconds)")
ax[0].set_ylabel("Attitude (radians)")

plot_sp(ax[1], attitude_df, attitude_sp_df, ["rollspeed", "pitchspeed", "yawspeed"], ["angle_vel_x", "angle_vel_y", "angle_vel_z"], start=start_time, end=end_time)    
