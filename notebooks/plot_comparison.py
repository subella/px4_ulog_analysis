...
def get_trajectory_bounds(dataframes, show_all=False, start_padding=3.0, end_padding=1.0):
...
def get_pos_errors(dataframes, bounds):
    ...
    for i in range(pos_start_idx, pos_end_idx):
        ...
        to_compare = lookup_time_index(dataframes["position"], dataframes["position_sp"]["times"][i])
...
def get_att_errors(dataframes, bounds):
    ...
    for i in range(att_start_idx, att_end_idx):
        ...
        to_compare = lookup_time_index(dataframes["attitude"], dataframes["attitude_sp"]["times"][i])
...
def plot_adaptive(ax, dataframes, name, fields, bounds, postfix="", **kwargs):
    ...
    ax.plot(dataframes[name].iloc[start_idx:end_idx]["times"] - bounds[0], deltas[start_idx:end_idx, 0], label=labels[0], c=colors[0], **kwargs)
    ax.plot(dataframes[name].iloc[start_idx:end_idx]["times"] - bounds[0], deltas[start_idx:end_idx, 1], label=labels[1], c=colors[1], **kwargs)
    ax.plot(dataframes[name].iloc[start_idx:end_idx]["times"] - bounds[0], deltas[start_idx:end_idx, 2], label=labels[2], c=colors[2], **kwargs)
...
adaptive_range = get_trajectory_bounds(adaptive_dfs, start_padding=start_padding, end_padding=end_padding)
geometric_range = get_trajectory_bounds(geometric_dfs, start_padding=start_padding, end_padding=end_padding)
...
# Adaptive terms
plot_adaptive(ax[0][1], adaptive_dfs, "attitude_sp", ["adap_x", "adap_y", "adap_z"], adaptive_range)
if show_geometric_adaptive_terms:
    plot_adaptive(ax[0][1], geometric_dfs, "attitude_sp", ["adap_x", "adap_y", "adap_z"], geometric_range, postfix=" (geometric)", ls="--")
...
plot_adaptive(ax[1][1], adaptive_dfs, "attitude_sp", ["adap_vx", "adap_vy", "adap_vz"], adaptive_range)
if show_geometric_adaptive_terms:
    plot_adaptive(ax[1][1], geometric_dfs, "attitude_sp", ["adap_vx", "adap_vy", "adap_vz"], geometric_range, postfix=" (geometric)", ls="--")
...
plot_adaptive(ax[2][1], adaptive_dfs, "attitude_output", ["adap_x", "adap_y", "adap_z"], adaptive_range)
if show_geometric_adaptive_terms:
    plot_adaptive(ax[2][1], geometric_dfs, "attitude_output", ["adap_x", "adap_y", "adap_z"], geometric_range, postfix=" (geometric)", ls="--")
